...
palindromify s = chop (isPalindrome s) s
    where chop False s = palindromify (init (tail s))
          chop True  s = s
...
safeIndex xs i = safe (i>=0 && i<(length xs)) i xs
    where safe True i xs = Just(head (drop i xs))
          safe False i xs = Nothing
